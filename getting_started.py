...
class ConxManager():
    ...
    def delete_tag(self,conx_id,tag_id):
        print('tags',self.tag_subs[conx_id]['tags'],self.connections[conx_id].polling)
        if not self.connections.get(conx_id):
            raise Exception(f"Fuck! no connection named {conx_id}")
    ...
    def disconnect(self, conx_id):
        #Need to check that we are disconnecting a connection that is connected

        if not self.connections.get(conx_id):
             raise Exception(f"Fuck! no connection named {conx_id}")
        for t in self.tag_subs[conx_id]['tags']:
            self.link.unsubscribe(f"[{conx_id}]{t.get('id')}", conx_id)
...
con_man.new_tag('Fred', {"id": f"Seconds", "description": f"Blah Blah","value":0,"address": 264 })
con_man.new_tag('Fred', {"id": f"Minutes", "description": f"Blah Blah","value":0,"address": 265 })
con_man.connect('Fred')
for x in range(5):
    print('data',con_man.get_data())
    time.sleep(0.5)
con_man.disconnect('Fred')
print('disconnected')

con_man.delete_tag('Fred', "Minutes")
con_man.connect('Fred')
for x in range(5):
    print('data',con_man.get_data())
    time.sleep(0.5)
con_man.disconnect('Fred')
#print('removed',con_man.get_data())
#print('result',con_man.is_polling('Fred'))
#print('result2',con_man.is_polling('Fredddy'))
#print('result3',con_man.is_tag('Fred','Seconds'))
#print('result4',con_man.is_tag('Fred','Minutes'))
